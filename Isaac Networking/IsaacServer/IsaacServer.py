...
# adapted functions from SolarPolarMan for seed generation.
...
def genSeed():
    ...
    for loopindex in range(0, 8):
        ...
        seedsprev = seedsprev >> 5
    return finalseed[:4]+" "+finalseed[4:] # add the space
...
def CloseConnection(client):
    global num_sim_clients
    num_sim_clients -= 1
    connected_clients.remove(client)
    del IP_to_buffer[client.getpeername()]
...
def InitiateConnections(connections_asked):
    ...
    for connection in connections_asked:  # connect the clients
        if num_sim_clients < config['maxClient']:
